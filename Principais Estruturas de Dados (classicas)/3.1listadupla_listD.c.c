...
// funcao que recebe uma lista, divide ela no meio, mantendo uma e criando uma nova
lista *divide_lista(lista *l) {
    lista *l2 = criar_lista_dupla();                             // Cria a lista dupla encadeada genérica
    int meio = l->tamanho / 2;                                   // Calcula o meio da lista
    no *atual = l->inicio;                                       // Inicializa o nó atual
    int i;

    for (i = 0; i < meio; i++) {                                 // Percorre a lista até o meio
        atual = atual->prox;
    }
    
    l2->inicio = atual;                                          // Atualiza o início da lista 2
    l2->fim = l->fim;                                            // Atualiza o fim da lista 2
    l2->tamanho = l->tamanho - meio;                             // Atualiza o tamanho da lista 2

    l->fim = atual->ant;                                         // Atualiza o fim da lista 1
    l->fim->prox = NULL;                                         // Atualiza o próximo do novo nó final da lista 1
    l->tamanho = meio;                                           // Atualiza o tamanho da lista 1
    return l2;                                                   // Retorna a lista 2
}
...
int main(){
    ...
    // Criar uma lista dupla encadeada genérica
    lista *l2 = criar_lista_dupla();                             // Cria a lista dupla encadeada genérica

    // Inserir n elementos no início da lista
    for(int i=0; i<5; i++){
        inserir_inicio_dupla(l2, i+1);
    }

    // Imprimir a lista
    printf("Inseridos 10 elementos no início da lista. \n");
    printf("Tamanho da lista: %d \n", l2->tamanho);               // Imprime o tamanho da lista
    imprimir_lista_dupla(l2);                                    // Imprime a lista
    //imprimir_lista_dupla_tras(l2);                               // Imprime a lista de trás para frente

    // Dividir a lista no meio
    lista *l3 = divide_lista(l2);                                // Divide a lista no meio

    // Imprimir as listas
    printf("Dividida a lista no meio. \n");
    printf("Tamanho da lista 1: %d \n", l2->tamanho);             // Imprime o tamanho da lista 1
    printf("Tamanho da lista 2: %d \n", l3->tamanho);             // Imprime o tamanho da lista 2
    imprimir_lista_dupla(l2);                                    // Imprime a lista 1
    //imprimir_lista_dupla_tras(l2);                               // Imprime a lista 1 de trás para frente
    imprimir_lista_dupla(l3);                                    // Imprime a lista 2
    //imprimir_lista_dupla_tras(l3);                               // Imprime a lista 2 de trás para frente

    liberar_lista_dupla(l2);                                     // Libera a memória alocada para a lista 1
    liberar_lista_dupla(l3);                                     // Libera a memória alocada para a lista 2
